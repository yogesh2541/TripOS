      ...
      {contact ? (
        <span className="inline-flex items-center gap-1.5">
          ...
          <button
            ...
            className="rounded-[6px] p-1 text-muted hover:bg-paper-2 hover:text-ink transition-colors"
        ...
        <button
          onClick={() => setOpen(true)}
          className="inline-flex items-center gap-2 rounded-[8px] border border-[var(--gold-line)] bg-gold-soft px-3.5 py-2 text-xs font-semibold text-gold-deep shadow-soft transition-colors hover:bg-gold-soft/80"
        >
          <span className="relative flex h-2 w-2">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--gold-line)] opacity-75" />
...
function LinkedChip({ contact }: { contact: { id: string; name: string } }) {
  return (
    <Link
      ...
      className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.2em] text-muted hover:text-ink transition-colors"
    >
                ...
                return (
                  <li key={l.id}>
                    <button
                      ...
                      className={`flex w-full items-center justify-between gap-3 rounded-[8px] border px-3.5 py-2.5 text-left transition-colors ${
                        isCurrent
                          ? "border-line bg-paper-2 cursor-default"
                          : "border-line bg-paper hover:border-[var(--gold-line)] hover:bg-paper-2 disabled:opacity-50"
                      }`}
                    >
                      ...
                      {isCurrent ? (
                        ...
                        <Link2 className="h-4 w-4 shrink-0 text-gold-deep" />
                      )}
                    </button>
                  </li>
                );
        ...
        {currentLeadId && (
          <div className="flex justify-between border-t border-line pt-4">
            <Button
              ...
              onClick={unlink}
              disabled={isPending}
              className="text-bad hover:text-bad hover:bg-bad-soft"
            >
              {busyId === "__unlink__" ? (
